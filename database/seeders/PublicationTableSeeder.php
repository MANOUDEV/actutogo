...
class PublicationTableSeeder extends Seeder
{
    ...
    {
        $perPage = 100;
        $response = Http::get("https://www.togoactualite.com/wp-json/wp/v2/posts?per_page=$perPage");
        ...
        for ($i = 1; $i <= $totalPages; $i++) {
            $posts = Http::get("https://www.togoactualite.com/wp-json/wp/v2/posts?page=$i&per_page=$perPage")->json();

            foreach ($posts as $value) {
                $author = Author::where('wp_author_id', $value['author'])->first();
                $typePublication = TypePublication::where('slug', 'article')->first();
            ...
            $this->command->info("Page $i de publications traitée.");
        }
