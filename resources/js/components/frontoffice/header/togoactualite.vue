...
const togoactualiteData = ref({});
const togoactualiteMessage = ref(null);
const empty = ref(2);

const getResults = async () => {
  loading.value = true;
  await store.dispatch('togoactualite/togoactualiteDataRequest', {});

  const status = store.getters['togoactualite/getInfosTogoactauliteStatus'];
  const message = store.getters['togoactualite/getInfosTogoactauliteMessage'];
  const data = store.getters['togoactualite/getInfosTogoactauliteData'];

  if (status === 'success') {
    togoactualiteData.value = data;
    empty.value = 0;
  } else {
    togoactualiteMessage.value = message;
...
<template>
  <div class="container">
    ...
    <div v-else class="row g-4 p-3 flex-fill">
      ...
      <div
        v-else-if="empty === 0"
        v-for="result in togoactualiteData"
        :key="result.id"
