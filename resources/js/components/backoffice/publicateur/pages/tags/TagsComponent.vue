...
const authFormModalTagsPubCreateClose = () => {
  ...
  $('#authFormModalTagsPubCreate').modal('hide')
}
...
const authModalClick = (action_auth = "LIST", modalName = null, modalSlug = null) => {
  ...
  $('#authFormModalTagsPubCreate').modal("show")
}
...
const submitLogin = async () => {
    ...
    } else if (loginStatus === 'success admin') {
    if (remember_me.value) {
        localStorage.setItem('username', username.value);
        localStorage.setItem('password', password.value);
        localStorage.setItem('remember_me', true);
...
const  getResultsSearchByDate = async() =>{
    ...
    await store.dispatch('tagsPub/searchByDateListDataRequest', {});

    const status = store.getters['tagsPub/getInfosSearchByDateListStatus'];
    const message = store.getters['tagsPub/getInfosSearchByDateListMessage'];
    const data = store.getters['tagsPub/getInfosSearchByDateListData'];
...
const  handleSelection = async(event, page=1) =>{
   ...
   searchH.value = event.target.value
    ...
    await store.dispatch('tagsPub/tagsListHDataRequest', {
        page, 
        searchH: searchH.value
    });

    const status = store.getters['tagsPub/getInfosTagsListStatus'];
    const message = store.getters['tagsPub/getInfosTagsListMessage'];
    const data = store.getters['tagsPub/getInfosTagsListData'];
...
const getResults = async (page = 1) => {
    ...
    await store.dispatch('tagsPub/tagsListDataRequest', {
    page,
    search: search.value
    });
...
const tagsCreate = async () => {
    ...
    await store.dispatch('tagsPub/tagsStoreDataRequest', { name: name.value });

    const status = store.getters['tagsPub/getInfosTagsStoreStatus'];
    const message = store.getters['tagsPub/getInfosTagsStoreMessage'];
    const data = store.getters['tagsPub/getInfosTagsStoreData'];
    const errors = store.getters['tagsPub/getInfosTagsStoreErrors'];
...
const tagsUpdate = async () => {
    ...
    await store.dispatch('tagsPub/tagsUpdateDataRequest', {
    name: name.value,
    slug: slug.value
    });

    const status = store.getters['tagsPub/getInfosTagsUpdateStatus'];
    const message = store.getters['tagsPub/getInfosTagsUpdateMessage'];
    const data = store.getters['tagsPub/getInfosTagsUpdateData'];
    const errors = store.getters['tagsPub/getInfosTagsUpdateErrors'];
...
const tagsDelete = async () => {
    ...
    await store.dispatch('tagsPub/tagsDeleteDataRequest', { slug: slug.value });

    const status = store.getters['tagsPub/getInfosTagsDeleteStatus'];
    const message = store.getters['tagsPub/getInfosTagsDeleteMessage'];
    const data = store.getters['tagsPub/getInfosTagsDeleteData'];
    const errors = store.getters['tagsPub/getInfosTagsDeleteErrors'];
...
const create = async () => {
  ...
  if (accessToken && nbRsp) {
    ...
    if (roleStatus === 'success') {
      ...
      if (meRoleName.value === nbRsp && nbRsp === '&nbrsp?!') {
        await tagsCreate();
      } else {
        loadingCreate.value = true;
      }
...
<template>
    ...
    <!-- Modal Form -->
    <div class="modal fade" id="authFormModalTagsPubCreate" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" v-if="authSectionModal == 'CREATE'">
                <div class="modal-header">
                    <h5 class="modal-title">Ajouter un tag</h5>
                    <button type="button" class="btn-close" @click="authFormModalTagsPubCreateClose" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form>
                    ...
                    <div class="modal-footer" >
                        <div  v-if="!loadingCreate" style="margin-top: -5px; margin-bottom: -10px">
                            ...
                            &nbsp;
                            <span><span class="btn btn-danger" @click="authFormModalTagsPubCreateClose">Fermez</span></span>
                        </div>
            ...
            <div class="modal-content" v-if="authSectionModal == 'DELETE'">
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex justify-content-center" v-if="!loadingDelete" style="margin-top: -5px; margin-bottom: -10px">
                        ...
                        &nbsp;
                        <span><span class="btn btn-success" @click="authFormModalTagsPubCreateClose">Annuler</span></span>
                    </div>
