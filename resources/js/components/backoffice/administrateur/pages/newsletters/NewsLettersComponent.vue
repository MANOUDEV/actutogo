...
const newsLetterBlockData = ref({})
const newsLetterBlockMessage = ref(null)


const newsLetterActivateData = ref({})
const newsLetterActivateMessage = ref(null)
...
const authModalClick = (action_auth = "LIST", modalName = null, modalSlug = null) => {
  ...
  }else if (authSectionModal.value === "ACTIVATE") {
    loadingActivate.value = false
  }else if (authSectionModal.value === "BLOCK") {
...
const submitLogin = async () => {
    ...
    if (authSectionModal.value === 'LIST') {
       ...
       newsLetterDelete();
    } else if (authSectionModal.value === 'ACTIVATE_AUTH') {
        authSectionModal.value = 'ACTIVATE';
       newsLetterActivate();
    } else if (authSectionModal.value === 'BLOCK_AUTH') {
...
const handleSelectionSearchByStatus = async (event,page = 1) => {
    ...
    const statut = store.getters['newsLetterAdmin/getInfosNewsLetterSearchStatus'];
...
const newsLetterCreate = async () => {
    ...
    await store.dispatch('newsLetterAdmin/newsLetterStoreDataRequest', { email: email.value });
...
<template>
                                        ...
                                        <tbody class="border-top-0">
                                            ...
                                            <tr v-for="result in newsLetterListData.newsLetters.data" :key="result.id">
                                                ...
                                                <td>
                                                    ...
                                                    <span v-else-if="result.count_publications == 1" class="badge bg-success bg-opacity-10 text-success mb-2">1 publication</span>
                                                    <span v-else class="badge bg-success bg-opacity-10 text-success mb-2"> {{ result.count_publications }} publications</span>
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    <span style="cursor: pointer" @click="authModalClick('BLOCK',result.email, result.slug )" v-else-if="result.status == 1" class="badge bg-success bg-opacity-10 text-light mb-2">Accepté</span>
    ...
    <!-- Modal Form -->
    <div class="modal fade" id="authFormModalNewsLetterCreate" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" v-if="authSectionModal == 'CREATE'">
                <div class="modal-header">
                    <h5 class="modal-title">Ajouter un abonné</h5>
                    <button type="button" class="btn-close" @click="authFormModalNewsLetterCreateClose" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form>
                    ...
                    <div class="modal-footer" >
                        <div  v-if="!loadingCreate" style="margin-top: -5px; margin-bottom: -10px">
                            ...
                            &nbsp;
                            <span><span class="btn btn-danger" @click="authFormModalNewsLetterCreateClose">Fermez</span></span>
                        </div>
            ...
            <div class="modal-content" v-if="authSectionModal == 'DELETE'">
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex justify-content-center" v-if="!loadingDelete" style="margin-top: -5px; margin-bottom: -10px">
                        ...
                        &nbsp;
                        <span><span class="btn btn-success" @click="authFormModalNewsLetterCreateClose">Annuler</span></span>
                    </div>
