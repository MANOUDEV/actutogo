...
const typePublicationListData = ref({})
const typePublicationListMessage = ref(null)

const typePublicationStoreData = ref({})
const typePublicationStoreMessage = ref(null)
const typePublicationStoreErrors = ref([])

const typePublicationUpdateData = ref({})
const typePublicationUpdateMessage = ref(null)
const typePublicationUpdateErrors = ref([])

const typePublicationDeleteData = ref({})
const typePublicationDeleteMessage = ref(null)
...
const authFormModalTypePublicationCreateClose = () => {
...
const getResults = async (page = 1) => {
    ...
    await store.dispatch('typePublicationAdmin/typePublicationListDataRequest', {
    page,
    search: search.value
    });

    const status = store.getters['typePublicationAdmin/getInfosTypePublicationListStatus'];
    const message = store.getters['typePublicationAdmin/getInfosTypePublicationListMessage'];
    const data = store.getters['typePublicationAdmin/getInfosTypePublicationListData'];

    if (status === 'success') {
    typePublicationListData.value = data;
    empty.value = 0;
    dataReady.value = 1;
    loading.value = false;
    } else if (status === 'empty') {
    typePublicationListData.value = data;
    typePublicationListMessage.value = message;
    empty.value = 1;
    dataReady.value = 1;
...
const typePublicationCreate = async () => {
    loadingCreate.value = true;
    authSectionModal.value = 'CREATE';

    await store.dispatch('typePublicationAdmin/typePublicationStoreDataRequest', { name: name.value });

    const status = store.getters['typePublicationAdmin/getInfosTypePublicationStoreStatus'];
    const message = store.getters['typePublicationAdmin/getInfosTypePublicationStoreMessage'];
    const data = store.getters['typePublicationAdmin/getInfosTypePublicationStoreData'];
    const errors = store.getters['typePublicationAdmin/getInfosTypePublicationStoreErrors'];

    if (status === 'success') {
    typePublicationStoreData.value = data;
    showToast('success',message);
    loadingCreate.value = false;
    name.value = null;
    slug.value = null;
    ...
    typePublicationStoreMessage.value = message;
    typePublicationStoreErrors.value = errors;
    loadingCreate.value = false;
    } else if (status === 'error') {
    typePublicationStoreMessage.value = message;
    typePublicationStoreErrors.value = [];
    loadingCreate.value = false;
    }
};

const typePublicationUpdate = async () => {
    loadingUpdate.value = true;
    authSectionModal.value = 'UPDATE';

    await store.dispatch('typePublicationAdmin/typePublicationUpdateDataRequest', {
    name: name.value,
    slug: slug.value
    });

    const status = store.getters['typePublicationAdmin/getInfosTypePublicationUpdateStatus'];
    const message = store.getters['typePublicationAdmin/getInfosTypePublicationUpdateMessage'];
    const data = store.getters['typePublicationAdmin/getInfosTypePublicationUpdateData'];
    const errors = store.getters['typePublicationAdmin/getInfosTypePublicationUpdateErrors'];

    if (status === 'success') {
    typePublicationUpdateData.value = data;
    showToast('success',message);
    loadingUpdate.value = false;
    name.value = null;
    slug.value = null;
    ...
    typePublicationUpdateMessage.value = message;
    typePublicationUpdateErrors.value = errors;
    loadingUpdate.value = false;
    } else if (status === 'error') {
    typePublicationUpdateMessage.value = message;
    typePublicationUpdateErrors.value = [];
    loadingUpdate.value = false;
    }
};

const typePublicationDelete = async () => {
    loadingDelete.value = true;
    authSectionModal.value = 'DELETE';

    await store.dispatch('typePublicationAdmin/typePublicationDeleteDataRequest', { slug: slug.value });

    const status = store.getters['typePublicationAdmin/getInfosTypePublicationDeleteStatus'];
    const message = store.getters['typePublicationAdmin/getInfosTypePublicationDeleteMessage'];
    const data = store.getters['typePublicationAdmin/getInfosTypePublicationDeleteData'];
    const errors = store.getters['typePublicationAdmin/getInfosTypePublicationDeleteErrors'];

    if (status === 'success') {
        typePublicationDeleteData.value = data;
        showToast('success',message);
        loadingDelete.value = false;
        name.value = null;
        slug.value = null;
        ...
        typePublicationDeleteMessage.value = message;
        typePublicationDeleteErrors.value = errors;
        loadingDelete.value = false;
    } else if (status === 'error') {
        typePublicationDeleteMessage.value = message;
        loadingDelete.value = false;
