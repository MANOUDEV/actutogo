...
const categoryListMessage = ref(null)

const categoryStoreData = ref({})
const categoryStoreMessage = ref(null)
const categoryStoreErrors = ref([])

const categoryUpdateData = ref({})
const categoryUpdateMessage = ref(null)
const categoryUpdateErrors = ref([])

const categoryDeleteData = ref({})
const categoryDeleteMessage = ref(null)
...
const authFormModalCategoryCreateClose = () => {
  ...
  $('#authFormModalCategoryCreate').modal('hide')
}
...
const authModalClick = (action_auth = "LIST", modalName = null, modalSlug = null) => {
  ...
  $('#authFormModalCategoryCreate').modal("show")
}
...
const getResults = async (page = 1) => {
    ...
    await store.dispatch('categoryAdmin/categoryListDataRequest', {
    page,
    search: search.value
    });

    const status = store.getters['categoryAdmin/getInfosCategoryListStatus'];
    const message = store.getters['categoryAdmin/getInfosCategoryListMessage'];
    const data = store.getters['categoryAdmin/getInfosCategoryListData'];

    if (status === 'success') {
    categoryListData.value = data;
    empty.value = 0;
    dataReady.value = 1;
    loading.value = false;
    } else if (status === 'empty') {
    categoryListData.value = data;
    categoryListMessage.value = message;
    empty.value = 1;
    dataReady.value = 1;
...
const categoryCreate = async () => {
    ...
    await store.dispatch('categoryAdmin/categoryStoreDataRequest', { name: name.value });

    const status = store.getters['categoryAdmin/getInfosCategoryStoreStatus'];
    const message = store.getters['categoryAdmin/getInfosCategoryStoreMessage'];
    const data = store.getters['categoryAdmin/getInfosCategoryStoreData'];
    const errors = store.getters['categoryAdmin/getInfosCategoryStoreErrors'];

    if (status === 'success') {
    categoryStoreData.value = data;
    showToast('success',message);
    loadingCreate.value = false;
    name.value = null;
    slug.value = null;
    ...
    categoryStoreMessage.value = message;
    categoryStoreErrors.value = errors;
    loadingCreate.value = false;
    } else if (status === 'error') {
    categoryStoreMessage.value = message;
    categoryStoreErrors.value = [];
    loadingCreate.value = false;
    }
};
...
const categoryUpdate = async () => {
    ...
    await store.dispatch('categoryAdmin/categoryUpdateDataRequest', {
    name: name.value,
    slug: slug.value
    });

    const status = store.getters['categoryAdmin/getInfosCategoryUpdateStatus'];
    const message = store.getters['categoryAdmin/getInfosCategoryUpdateMessage'];
    const data = store.getters['categoryAdmin/getInfosCategoryUpdateData'];
    const errors = store.getters['categoryAdmin/getInfosCategoryUpdateErrors'];

    if (status === 'success') {
    categoryUpdateData.value = data;
    showToast('success',message);
    loadingUpdate.value = false;
    name.value = null;
    slug.value = null;
    ...
    categoryUpdateMessage.value = message;
    categoryUpdateErrors.value = errors;
    loadingUpdate.value = false;
    } else if (status === 'error') {
    categoryUpdateMessage.value = message;
    categoryUpdateErrors.value = [];
    loadingUpdate.value = false;
    }
};
...
const categoryDelete = async () => {
    ...
    await store.dispatch('categoryAdmin/categoryDeleteDataRequest', { slug: slug.value });

    const status = store.getters['categoryAdmin/getInfosCategoryDeleteStatus'];
    const message = store.getters['categoryAdmin/getInfosCategoryDeleteMessage'];
    const data = store.getters['categoryAdmin/getInfosCategoryDeleteData'];
    const errors = store.getters['categoryAdmin/getInfosCategoryDeleteErrors'];

    if (status === 'success') {
        categoryDeleteData.value = data;
        showToast('success',message);
        loadingDelete.value = false;
        name.value = null;
        slug.value = null;
        ...
        categoryDeleteMessage.value = message;
        categoryDeleteErrors.value = errors;
        loadingDelete.value = false;
    } else if (status === 'error') {
        categoryDeleteMessage.value = message;
        loadingDelete.value = false;
...
<template>
                ...
                <div class="row g-4 pb-4">
                    <div class="col-12">
                        ...
                        <div class="d-sm-flex justify-content-sm-between align-items-center">
                            <h1 class="mb-2 mb-sm-0 h2">Mes catégories <span class="badge bg-primary bg-opacity-10 text-primary"> {{ categoryListData.categoryCount  }} </span></h1>
                            <span style="cursor: pointer" @click="authModalClick('CREATE')" class="btn btn-sm btn-primary mb-0"><i class="fas fa-plus me-2"></i>Ajouter une catégorie</span>
                        </div>
                    </div>
                </div>
                <div class="row g-4">
                                ...
                                <div class="row g-3 align-items-center justify-content-between">
                                    ...
                                    <div class="col-md-10 col-sm-10 col-xs-10">
                                        <form class="rounded position-relative" method="GET" @submit.prevent="getResults">
                                            <input class="form-control bg-transparent" v-model="search" name="search" @input="show" type="search" placeholder="Rechercher une catégorie ..." aria-label="Search">
                            ...
                            <div class="card-body p-3 pb-0" v-if="empty == 0">
                                <div class="tab-content py-0 my-0">
                                        ...
                                        <div class="table-responsive border-0">
                                            <table class="table align-middle p-4 mb-0 table-hover">
                                                ...
                                                <tbody class="border-top-0">
                                                    <!-- Table row -->
                                                    <tr v-for="result in categoryListData.categories.data" :key="result.id">
                                    ...
                                    <div class="tab-pane fade show active" id="nav-grid-tab">
                                        <div class="row g-4">
                                            <div class="col-md-6 col-xl-4" v-for="result in categoryListData.categories.data" :key="result.id">
                                                <!-- Category item START -->
                            ...
                            <div class="card-body p-3 pb-0" v-else-if="empty == 1">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                      ...
                                      <h5 style="text-align: center; margin-top: -50px"> {{ categoryListMessage  }} </h5>
                                    </div>
                                    <div class="col-md-3"></div>
                                </div>
                            </div>
                            ...
                            <div class="card-footer p-3" v-if="empty == 0">
                                ...
                                <div class="d-sm-flex justify-content-sm-between align-items-sm-center">
                                    ...
                                    <Bootstrap5Pagination
                                        class="mb-0"
                                        :data="categoryListData.categories"
                                        :limit="limit"
    ...
    <!-- Modal Form -->
    <div class="modal fade" id="authFormModalCategoryCreate" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" v-if="authSectionModal == 'CREATE'">
                <div class="modal-header">
                    <h5 class="modal-title">Ajouter une catégorie</h5>
                    <button type="button" class="btn-close" @click="authFormModalCategoryCreateClose" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form>
                    <div class="modal-body">
                        <div v-if="categoryStoreMessage">
                            <div class="alert alert-danger"  role="alert">
                                {{ categoryStoreMessage }}
                            </div>
                        </div>

                        <!-- Email -->
                        <div class="mb-3" v-if="categoryStoreErrors.name">
                            <label class="form-label" for="exampleInputEmailCategoryCreate">Catégorie</label>
                            <input type="text" v-model="name" name="name" class="form-control is-invalid" id="exampleInputEmailCategoryCreate" placeholder="Entrez la catégorie">
                            <div v-for="errorname in categoryStoreErrors.name" :key="errorname" class="invalid-feedback">
                                {{ errorname }}
                            </div>
                        </div>
                        <div class="mb-3" v-else>
                            <label class="form-label" for="exampleInputEmailLoginInvalid">Catégorie</label>
                            <input type="text" v-model="name" name="name" class="form-control" id="exampleInputEmailCategoryCreate" placeholder="Entrez la catégorie">
                        </div>
                    </div>
                    <div class="modal-footer" >
                        <div  v-if="!loadingCreate" style="margin-top: -5px; margin-bottom: -10px">
                            ...
                            &nbsp;
                            <span><span class="btn btn-danger" @click="authFormModalCategoryCreateClose">Fermez</span></span>
                        </div>
            ...
            <div class="modal-content" v-if="authSectionModal == 'UPDATE'">
                ...
                <form>
                    <div class="modal-body">
                        <div v-if="categoryUpdateMessage">
                            <div class="alert alert-danger"  role="alert">
                        ...
                        <!-- Email -->
                        <div class="mb-3" v-if="categoryUpdateErrors.name">
                            <label class="form-label" for="exampleInputEmailCategoryCreate">Catégorie</label>
                            <input type="text" v-model="name" name="name" class="form-control is-invalid" id="exampleInputEmailCategoryCreate" placeholder="Entrez la catégorie">
                            <div v-for="errorname in categoryUpdateErrors.name" :key="errorname" class="invalid-feedback">
                                {{ errorname }}
                            </div>
                        </div>
                        <div class="mb-3" v-else>
            ...
            <div class="modal-content" v-if="authSectionModal == 'DELETE'">
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex justify-content-center">
                        <p class="text-center">Toutes données relatives à ce Catégorie seront aussi supprimées.</p>
                    </div>
                    <div class="d-flex justify-content-center" v-if="!loadingDelete" style="margin-top: -5px; margin-bottom: -10px">
                        ...
                        &nbsp;
                        <span><span class="btn btn-success" @click="authFormModalCategoryCreateClose">Annuler</span></span>
                    </div>
